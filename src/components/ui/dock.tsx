  ...
  return (
    ...
    >
      <motion.div
        ref={dockRef}
        className={cn(
          "flex items-end h-14 p-2 gap-3 bg-opacity-90 rounded-xl",
        ...
        onMouseMove={(e) => {
          mouseX.set(e.pageX);
          if (!isZooming.current) setHovered(true);
        }}
        onMouseLeave={() => {
          mouseX.set(Infinity);
          setHovered(false);
...
function DockCard({ children, id, onClick }: DockCardProps) {
  ...
  const handleClick = () => {
    ...
    if (onClick) onClick(); // <-- call the parent click
  };
  ...
  // Animation controls for managing card's Y position during the animation loop
  ...
  // Reactive effect: start/stop bouncing based on animatingIndexes
  useEffect(() => {
    if (dock.animatingIndexes.includes(parseInt(id))) {
      controls.start({
        y: -24,
  ...
  }, [dock.animatingIndexes, controls, id]);
