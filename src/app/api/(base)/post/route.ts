...
export async function GET(request:NextRequest) {
  ...
  const id = url.searchParams.get("from");
    ...
    const { data: dbData, error } = await supabase.from("post").select(`*,user:user_id(
          name,
          role,
          alias,
          email,
...
export async function POST(req: NextRequest) {
  ...
  if (!data.body || typeof data.body !== "string") {
    return NextResponse.json({message:"Please provide a valid post body",ok:false},{status:422})
  }
